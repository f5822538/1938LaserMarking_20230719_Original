Module modLibrary
    ...
    Public Function ModelDiffForStandardDeviation(oCameraSourceImage As MIL_ID, oRecipe As CRecipeModelDiff, oInspectSum As CInspectSum, oProduct As CMyProduct, oMyEquipment As CMyEquipment, oLog As II_LogTraceExtend, nSequence As Integer, bIsSaveImage As Boolean, nDefectMaxCount As Integer) As Boolean
        Try
            ...
            Task.WaitAll(oFindModelAllTask1St, oFindModelAllTask2Nd) '等候-FindModelAll
    ...
    Public Function FindModelAll(oCameraSourceImage As MIL_ID, ByRef oModelImageList As List(Of CMyModelImage), oRecipe As CRecipeModelDiff, oInspectSum As CInspectSum, oProduct As CMyProduct, oLog As II_LogTraceExtend, oPatternMatchingType As PatternMatchingType, nSequence As Integer, bIsSaveImage As Boolean) As Boolean
        Try
            ...
            If oProduct IsNot Nothing Then Parallel.ForEach(oModelImageList, Sub(o)
                                                                                 BuildProductPosition(o, oRecipe, oInspectSum, oProduct, oPatternMatchingType, oLog, nSequence) '蓋印漏雷/蓋印轉置,位移/偏移,蓋印不清
                                                                             End Sub)
            ...
            With oInspectSum
                ...
                If oModelImage.IsProcess = False Then '大部分是走這個流程(oModelImage.IsProcess = False)
                    ...
                    If oModelImage.IsOffset = True Then '位移/偏移
                        ...
                        oDefect.DefectName = EnumHelper.GetDescription(oProduct.MarkList.Item(nIndex).Result) '瑕疵名稱(可用於出報表)-位移/偏移
                        oDefect.MeanGray = 0
                        ...
                        Return False '在結果Result列表顯示-瑕疵:0,檢測異常(樣板):Y,顯示為紅字原因為何[oModelImage.IsOffset = True,位移/偏移]
                    ElseIf oModelImage.IsOffsetGray = True Then '標記-位移/偏移(灰階)
                        ...
                        oDefect.DefectName = EnumHelper.GetDescription(oProduct.MarkList.Item(nIndex).Result) '瑕疵名稱(可用於出報表)-位移/偏移(灰階)
                        oDefect.MeanGray = 0
                        oDefect.BodyArea = oRecipe.ModelSize.Width * oRecipe.ModelSize.Height
                        oDefect.DefectArea = CInt(oDefect.BodyArea * oMyEquipment.HardwareConfig.CameraConfig.PixelSize)
                        oDefect.DefectBoundary.Top = oModelImage.PositionY
                        ...
                        Return False '在結果Result列表顯示-瑕疵:0,檢測異常(樣板):Y,顯示為紅字原因為何[oModelImage.IsOffsetGray = True,位移/偏移(灰階)]
                    ElseIf oModelImage.IsLose = True Then '蓋印漏雷/蓋印轉置
                        ...
                        Return False '在結果Result列表顯示-瑕疵:0,檢測異常(樣板):Y,顯示為紅字原因為何[oModelImage.IsLose = True,蓋印漏雷/蓋印轉置]
                ...
                ElseIf oModelImage.IsProcess = True Then
                    ...
                    If nIndex < 0 Then
                        ...
                        Return False '在結果Result列表顯示-瑕疵:0,檢測異常(樣板):Y,顯示為紅字原因為何[oModelImage.IsProcess = True, oProduct.MarkIndex(oModelImage.MarkX, oModelImage.MarkY) < 0]
                    End If
                    ...
                    If bIsIndistinct = True Then '標記-蓋印不清
                        ...
                        If oSelectModelImageList2Nd.Count < 1 Then
                            ...
                            oDefect.DefectName = EnumHelper.GetDescription(oProduct.MarkList.Item(nIndex).Result) '瑕疵名稱(可用於出報表)-蓋印不清
                            oDefect.MeanGray = 0
                            oDefect.BodyArea = oRecipe.ModelSize.Width * oRecipe.ModelSize.Height
                            oDefect.DefectArea = CInt(oDefect.BodyArea * oMyEquipment.HardwareConfig.CameraConfig.PixelSize)
                            oDefect.DefectBoundary.Top = oModelImage.PositionY
                            ...
                            Return False '在結果Result列表顯示-瑕疵:0,檢測異常(樣板):Y,顯示為紅字原因為何[oModelImage.IsProcess = True, bIsIndistinct = True, oSelectModelImageList2Nd.Count < 1]
                        End If
                    End If
                End If
                ...
                Parallel.For(0, nResultCountPositive + 1, Sub(nIndexPositive As Integer)
                                                              ...
                                                              oDefect.DefectName = EnumHelper.GetDescription(If(oDefectType = ResultType.NGDark, ResultType.NGDark, ResultType.NGBright)) '瑕疵名稱(可用於出報表)-表面瑕疵
